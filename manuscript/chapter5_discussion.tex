...
The $160\times40$ validation run (600 iterations) gives
RMSE~$=\rmseHiRes=0.295$.
The expected monotone decrease is not observed (80$\times$20: 0.079,
160$\times$40: 0.295), consistent with the gray-design
finding: the optimizer finds different local minima at different
mesh resolutions. Systematic mesh convergence requires a fixed
continuation schedule tuned per resolution.
...
consistency gap: the computed sensitivity is not the exact gradient of
the \emph{discretised} objective.
The Taylor test (Section~\ref{sec:taylor_test}) confirms that this
