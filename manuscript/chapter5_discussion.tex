...
\begin{itemize}
    ...
    \item \textbf{Additional target profiles.}
          Section~\ref{sec:profiles} demonstrates the framework
...
\begin{itemize}\itemsep2pt
    ...
    \item \textbf{Gradient magnitude}: the continuous adjoint direction
          is approximately correct (Pearson correlation $\geq 0.80$
          with finite-difference gradients) but the magnitude is wrong
          by factors of $10^2$--$10^4$ depending on mesh resolution.
          The OC/MMA optimisers are insensitive to magnitude errors
          (OC uses sign only; MMA normalises by bounds), so convergence
          is achieved despite the mismatch.
          A discrete adjoint with correct magnitude is future work.
    ...
    \item \textbf{Threshold-projected performance}: threshold-projected
          RMSE ($\rmseBinaryBrinkman$) reflects the loss of gray mixing
          pathways upon binarisation; the framework optimises within
          the Brinkman--SIMP porous-medium design space.
