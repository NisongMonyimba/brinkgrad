...
The classic ``Christmas tree'' network introduced by Jeon \emph{et~al.}~\cite{jeon2000} demonstrated that multiple fluid streams, repeatedly split and recombined in a planar network of microchannels, can produce a smooth, predictable concentration profile at the outlet.
...
\begin{enumerate}
    ...
    \item Demonstrating that the optimised designs outperform the classic Christmas tree generator by \textbf{\reduction\%} in hydraulic resistance (from \($\treeHydRes$\) to \($\hydResTopOpt$\)~Pa\(\cdot\)s/m\(^2\)) while maintaining equivalent outlet concentration fidelity
