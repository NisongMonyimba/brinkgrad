...
\begin{enumerate}
    ...
    \item Demonstrating that the optimised designs outperform the classic Christmas tree generator by \textbf{99.7\%} in hydraulic resistance (from \(1.77\times10^{11}\) to \(4.48\times10^{8}\)~Pa\(\cdot\)s/m\(^3\)) while maintaining equivalent or better outlet concentration linearity (RMSE~=~0.098).
