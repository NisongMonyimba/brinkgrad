...
\begin{enumerate}
    ...
    \item Demonstrating that the optimised designs outperform the classic Christmas tree generator by \textbf{\reduction\%} in hydraulic resistance (from \($\treeHydRes$\) to \($\hydResTopOpt$\)~Pa\(\cdot\)s/m\(^2\)) while producing a substantially closer approximation to the prescribed linear profile (topology-optimised RMSE~$=\rmseTopOpt$; Christmas-tree RMSE~$=\treeRMSE$).
