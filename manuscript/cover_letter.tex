...
\begin{center}
\textbf{\texttt{brinkgrad}: An open-source FEniCSx framework for adjoint-based topology optimisation of coupled flow-transport in porous media using Brinkman--convection-diffusion equations}
\end{center}

\textbf{Summary of contributions.}
This paper presents \texttt{brinkgrad}, an open-source FEniCSx framework
...
via finite-difference direction test (Pearson corr.\ $\geq0.80$)
(Pearson correlation $\geq 0.80$ with finite-difference gradients).

\item \textbf{Documented default configuration.}
Five documented parameters --- Helmholtz PDE filtering,
...
profiles (linear RMSE~$=0.058$, step RMSE~$=0.011$,
double-peak RMSE~$=0.007$) demonstrate that the framework adapts to arbitrary
...
increases RMSE from $0.058$ to $\rmseBinaryBrinkman$, confirming that the optimised
...
gradient direction verification and mesh sensitivity
consistent with non-convex topology optimisation behaviour.
...
reproducibility infrastructure, and a documented default configuration ---
