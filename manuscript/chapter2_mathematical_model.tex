...
All remaining boundaries are impermeable no-slip walls
($\mathbf{u}=\mathbf{0}$, zero concentration flux).
...
Table~\ref{tab:notation} summarises the three-field convention
used throughout.

\begin{table}[h]
\centering\small
\caption{Design variable notation used throughout the paper.}
\label{tab:notation}
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Symbol} & \textbf{Meaning} & \textbf{Range} \\
\midrule
$\rho$ & Raw design variable (optimised by OC/MMA) & $[0,1]$ \\
$\tilde{\rho}$ & Filtered density (after Helmholtz PDE filter) & $[0,1]$ \\
$\bar{\rho}$ & Physical density (after Heaviside projection) & $[0,1]$ \\
\bottomrule
\multicolumn{3}{@{}l}{\footnotesize
  All PDEs use $\bar{\rho}$; sensitivity is assembled w.r.t.\ $\bar{\rho}$
  and propagated to $\rho$ via the filter--projection chain.}
\end{tabular}
\end{table}
...
\paragraph{Intermediate-density behaviour and scope.}
\label{para:leakage}
...
through intermediate-density solid regions.
\texttt{brinkgrad} implements the classical Brinkman--SIMP formulation
and therefore inherits this intermediate-density behaviour.
The package includes robust projection utilities for post-processing
manufacturability studies, but manufacturable binary design generation
