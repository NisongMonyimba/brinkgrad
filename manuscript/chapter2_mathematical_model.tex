...
\begin{equation}
    ...
    \label{eq:Dinterp}
\end{equation}
with \(D_{\text{fluid}} = 10^{-9}\ \text{m}^2/\text{s}\) the molecular diffusivity of the solute, \(D_{\min}=10^{-9}\ \text{m}^2/\text{s}\) a small but non-zero lower bound (to prevent a singular stiffness matrix in solid regions), and \(p=3\) the SIMP exponent.
...
\begin{align}
    ...
    \frac{\partial D}{\partial\rho} &= p\,(D_{\text{fluid}} - D_{\min})\,\rho^{\,p-1}.
        \label{eq:dD}
\end{align}
Equation~\eqref{eq:sensitivity} is evaluated by first solving the forward problem~\eqref{eq:brinkman}--\eqref{eq:convdiff} to obtain \((\mathbf{u},p,c)\), then solving the adjoint problems~\eqref{eq:adj_flow_mom}--\eqref{eq:adj_conc_bc} for \((\mathbf{v},q,\lambda)\), and finally assembling the volume integral.
