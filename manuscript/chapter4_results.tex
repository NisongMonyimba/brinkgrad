...
gives RMSE~$=\rmsePrimary600$ with gray fraction $\grayPrimary600$;
...
\begin{figure}[h]
    ...
    \caption{Outlet concentration profiles for three target functions
             ($80\times20$ mesh, 600-iter hybrid OC/MMA).
             Left: sigmoid (RMSE~$=\rmseSigmoid$);
             right: Gaussian peak (RMSE~$=\rmseGaussian$).
             Linear target result is shown in Figure~\ref{fig:topology}.
             All three cases use the same adjoint structure.}
    \label{fig:profiles}
\end{figure}
