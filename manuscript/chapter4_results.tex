...
\begin{enumerate}
\item \textbf{Direction accuracy.}
      The Pearson correlation between $g$ and the
      finite-difference gradient is $\geq 0.80$,
...
\item \textbf{Magnitude accuracy.}
      The Euclidean gradient $\hat{g}=M^{-1}g$
      is obtained by solving the consistent mass system
$M\hat{g}=g$ (CG/Jacobi, $\mathrm{rtol}=10^{-10}$).
      This reduces the relative magnitude error from $>100\%$
      (Riesz representative) to $\approx 31\%$ on the
      $20\times5$ mesh.
      The residual error reflects the Helmholtz filter
      sensitivity chain, not the adjoint derivation
      (see Supplementary~S10 for details).
\end{enumerate}
