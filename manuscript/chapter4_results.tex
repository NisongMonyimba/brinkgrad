...
\paragraph{Direction test: consistent adjoint providing reliable descent direction.}
...
\caption{Adjoint gradient verification summary.
         ...
         Pearson correlation $\geq 0.80$: correct descent direction.
         Rel.~err: magnitude mismatch (expected for continuous adjoint;
         harmless for OC/MMA --- see Table~\ref{tab:scaling}).}
...
\paragraph{Numerical evidence: OC insensitivity to gradient scaling.}
To verify this claim empirically, we repeated the OC optimisation
on the $20\times5$ mesh with the sensitivity vector scaled by
factors of $10^{-2}$, $10^{-1}$, $1$, $10$, $10^2$, and $10^4$.
Table~\ref{tab:scaling} shows that best-$J$ reduction (0.1--0.4\%
over 60 iterations) is essentially identical across six orders of
magnitude of gradient scaling, confirming that OC convergence
depends on gradient \emph{sign}, not magnitude.
...
Both optimisers therefore achieve reliable convergence despite
the continuous-adjoint magnitude error, as confirmed by the
monotone best-$J$ trajectory in Figure~\ref{fig:convergence}.
