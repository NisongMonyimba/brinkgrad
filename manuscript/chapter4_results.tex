...
$160\times40$ & 6400 & 41 & $\rmseHiRes$  & (see S1) & validation \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[t]
    ...
    \caption{Pareto fronts for topology optimisation (blue squares,
             $w_f$ sweep) and Christmas-tree network (red circles,
             channel-width sweep) on the same $80\times20$ Brinkman mesh.
             Left: hydraulic resistance vs.\ outlet RMSE (log scale).
             Right: viscous dissipation $J_f$ vs.\ outlet RMSE.
             The topology optimiser achieves lower RMSE than the best-optimised Christmas tree at comparable resistance,
             confirming a genuine performance advantage over the
             best-achievable Christmas-tree design at each operating point.}
...
To ensure a fair comparison, we sweep the channel width over
$\{5,8,12,18,25,35,50,70\}\,\mu$m, tracing the Christmas-tree
Pareto front in (resistance, RMSE) space rather than comparing against
a single fixed, unoptimised configuration.
The topology optimiser Pareto front is traced by sweeping
$w_f \in \{10^{-4}, 5\times10^{-4}, 10^{-3}, 5\times10^{-3},
10^{-2}\}$ with $w_c = 5\times10^1$ fixed.
Figure~\ref{fig:comparison} shows both Pareto fronts.
