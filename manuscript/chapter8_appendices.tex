...
$D_{\min}$ sensitivity: reducing $D_{\min}$ from $10^{-15}$ to
$10^{-18}\,\text{m}^2\,\text{s}^{-1}$ on the near-binary design changes
the mean solid diffusivity from $7.3\times10^{-14}$ to
$\approx10^{-17}\,\text{m}^2\,\text{s}^{-1}$ (a factor of $10^3$),
with a corresponding $t_{\mathrm{diff}}/t_{\mathrm{flow}}$ ratio
exceeding $10^6$.
In both cases the outlet concentration profile is indistinguishable,
confirming that $D_{\min}$ does not create artificial transport
pathways in the converged binary design.

\noindent\emph{Note:} full binary-design thresholding results
(with $D=0$ in solid) require a body-fitted mesh to avoid a degenerate
purely convective problem in solid elements; this is implemented in
\texttt{ns\_validation.py} and will be reported in a companion study.
