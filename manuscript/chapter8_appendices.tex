...
\caption{Mesh convergence for the linear gradient target.
         ...
         local minima found). The $40\times10$ result shows optimizer
         sensitivity rather than mesh convergence.
...
The hybrid OC/MMA strategy (OC for topology formation, MMA for
sharpening) converges to $J = \finalJ$ with outlet RMSE~$= \rmseTopOpt$.
Pure OC exhibits period-2 oscillations at high $\beta$ due to
near-symmetric sensitivity distributions (47\%/53\% negative/positive
split); MMA resolves this via its asymptote-based move control.
The self-contained Svanberg dual-bisection MMA implementation requires
no external packages beyond those already in the FEniCSx environment.
