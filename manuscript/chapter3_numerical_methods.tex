...
The design is updated using the hybrid OC/MMA configuration.
...
(RMSE~$=\rmsePureMMA$, gray fraction $\grayPureMMA$
after 600 iterations, Table~\ref{tab:mma_comparison}),
because MMA cannot escape the uniform-density plateau
without the larger OC move at $\beta=1$.
OC with its bisection-based update provides the large
initial displacement needed; MMA then stabilises
the late-phase $\beta$-sharpening.
...
Hybrid OC/MMA & Pure MMA stagnates at $J\approx J_0$ for 600 iter (RMSE~$=\rmsePureMMA$, Table~\ref{tab:mma_comparison}); OC escapes the initial plateau & Validated by direct comparison \\ 
...
\begin{table}[h]
\centering\small
\caption{Hybrid OC/MMA vs.\ pure MMA: 600-iteration comparison
         on the $80\times20$ mesh, linear gradient target.
         Pure MMA stagnates near the initial objective;
         the hybrid schedule is validated by this comparison.}
\label{tab:mma_comparison}
\begin{tabular}{@{}lccc@{}}
\toprule
Schedule & RMSE & $J_{\rm best}$ & Gray fraction \\
\midrule
Pure MMA (600 iter) & $\rmsePureMMA$ & $2.78\times10^{-3}$ & $\grayPureMMA$ \\
Hybrid OC/MMA (600 iter) & $\rmseShortRun$ & $\ensuremath{1.77\times10^{-3}}$ & $\grayShortRun$ \\
Hybrid OC/MMA (1400 iter) & $\rmseTopOpt$ & $\finalJ$ & $\grayFrac$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering\small
\caption{Algorithm~1: Hybrid OC/MMA optimisation loop with
         $\beta$-continuation.
...
\url{https://github.com/NisongMonyimba/micrograd}~\cite{micrograd2025zenodo}.
...
A Zenodo archive (DOI:~\href{https://doi.org/10.5281/zenodo.20479523}{10.5281/zenodo.20479523}, )
...
V* sweep (the volume fraction sweep ($V^*=0.5$ optimal)) &
...
\caption{Software architecture of \texttt{micrograd}.
...
\texttt{micrograd} is a single flat Python package
(\texttt{micrograd/}) with one public entry point:
...
(i)~install dependencies and \texttt{micrograd} in editable mode
...
\begin{enumerate}\itemsep2pt
    ...
    \item \textbf{Zenodo archival.}
          ...
          DOI:~\href{https://doi.org/10.5281/zenodo.20479523}%
          {10.5281/zenodo.20479523}~\cite{micrograd2025zenodo},
...
software quality indicators for the \texttt{micrograd} package.

\begin{table}[h]
\centering
\caption{Software quality metrics for \texttt{micrograd} v1.0.0.
         LOC counts non-empty, non-comment source lines
         in \texttt{micrograd/*.py}.
         Unit tests are in \texttt{tests/test\_import.py};
         integration tests run inline in the CI pipeline.}
\label{tab:sw_metrics}
\begin{tabular}{@{}lrl@{}}
\toprule
\textbf{Metric} & \textbf{Value} & \textbf{Notes} \\
\midrule
Lines of code (LOC)          & 1{,}892 & \texttt{micrograd/*.py}, non-empty non-comment \\
Public functions / classes   & 68      & across 9 modules \\
Unit tests                   & 8       & \texttt{tests/test\_import.py}, pytest \\
CI integration tests         & 7       & inline in \texttt{ci.yml}, full API coverage \\
Total automated tests        & 15      & run on every \texttt{main} branch push \\
CI workflows                 & 1       & \texttt{.github/workflows/ci.yml} \\
Example scripts              & 7       & \texttt{examples/}: linear, step, double-peak, \\
                             &         & gallery, Christmas-tree, convergence study, \\
                             &         & robust projection demo \\
Reproducibility script       & 1       & \texttt{run\_all.sh} (52 lines, Docker-native) \\
Docker image                 & Yes     & \texttt{dolfinx/dolfinx:v0.7.3} (pinned) \\
Zenodo archival              & Yes     & DOI:~\href{https://doi.org/10.5281/zenodo.20479523}{10.5281/zenodo.20479523} \\
Open-source licence          & MIT     & \href{https://github.com/NisongMonyimba/micrograd}{GitHub} \\
Runtime (80$\times$20, 1 iter) & $\sim$1\,s & 8-core laptop, Docker container \\
Peak memory (20$\times$5, 50 iter) & 29.5\,MB & \texttt{tracemalloc} measurement \\
Test coverage                & 8.7\%  & import-level; full pipeline via CI \\
...
\texttt{micrograd} is designed for extensibility through three patterns.
...
All 68 public functions are documented in the API reference.
...
floating-point precision, confirming bit-reproducibility
within the pinned Docker environment.
...
All 15 tests pass on every push to the \texttt{main} branch,
as shown by the CI badge in the repository README.
Import-level test coverage is $8.7\%$;
the full optimisation pipeline
(forward solve, adjoint, OC/MMA update, $\beta$-continuation)
is exercised by the CI regression test (Test~7).
