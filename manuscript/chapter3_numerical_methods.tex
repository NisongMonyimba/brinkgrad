...
\emph{Note:} in the coarse-mesh (\(20\times5\)) runs reported here, \(\alpha_{\max}\) is held constant at \(10^{9}\,\text{Pa}\cdot\text{s}/\text{m}^2\) and symmetry breaking is instead achieved by the sinusoidal initialisation described above (Eq.~\eqref{eq:sinusoidal_init}).
...
\begin{itemize}
    ...
    \item A continuous-integration (CI) workflow (GitHub Actions~\cite{GitHubActions2024}) that automatically executes a three-stage pipeline on every commit: (i)~dependency installation inside the official \texttt{dolfinx/dolfinx:v0.7.3} Docker container, preserving the binary ABI compatibility required by the pre-compiled \textsc{PETSc} and \texttt{petsc4py} libraries by pinning \texttt{nlopt==2.7.1} and suppressing \textsc{NumPy} upgrades; (ii)~a forward-solver smoke test that asserts the concentration field satisfies $c_h\in[0,1]$ on a $20\times5$ mesh; and (iii)~a ten-iteration mini-optimisation that verifies $J<10$, confirming end-to-end correctness of the adjoint sensitivity, OC update, and $\alpha$-continuation mechanism. During development, this pipeline identified and resolved three API incompatibilities introduced in \texttt{dolfinx~v0.7}: removal of the \texttt{petsc\_options\_prefix} keyword from \texttt{LinearProblem}, promotion of \texttt{element.interpolation\_points} from a property to a callable method, and a \textsc{NumPy}~2.0 binary incompatibility introduced by \texttt{nlopt}\,$\geq$\,2.8. All three were caught by the CI pipeline before affecting any scientific result. The workflow definition (\texttt{.github/workflows/ci.yml}) is version-controlled and constitutes a machine-readable specification of the software environment required to reproduce all results;
