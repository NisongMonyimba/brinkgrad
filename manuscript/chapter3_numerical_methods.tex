...
(full checklist: Table~\ref{tab:repro_checklist}):
...
Coverage (\texttt{utilities.py})  & 52\%   & pure-Python; measured locally \\
Coverage (FEniCSx modules)        & CI     & solver, adjoint, optimiser validated\\
                                  &        & via 8 integration tests on every push \\
...
\caption{Mesh scaling for the linear gradient benchmark
         ...
         Runtime scales approximately linearly with mesh DOFs.}
\label{tab:scaling}
\begin{tabular}{@{}lrrrl@{}}
\toprule
...
\multicolumn{5}{@{}l}{\footnotesize
  DOFs = velocity (2$\times$mesh nodes) + pressure + concentration.
  Runtime $\propto$ DOFs$^{1.1}$ (near-linear; dominated by PETSc solves).}
\end{tabular}
\end{table}
