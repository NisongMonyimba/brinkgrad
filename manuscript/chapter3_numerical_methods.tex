...
All primary results are reproducible from three commands:
...
# Expected: RMSE=0.058 after ~1 hour on 80x20 mesh (standard laptop)
\end{lstlisting}
To optimise a different target profile, change one argument:
\texttt{target\_expr=lambda~x:~np.where(x[1]>250e-6,~1.0,~0.0)}
(step profile; all other settings identical).
...
All results are reproduced by \texttt{bash run\_all.sh}
inside the Docker container (see \texttt{examples/} for individual scripts).
...
Test coverage                & 52\%   & utilities.py; FEniCSx modules via CI pipeline \\
