...
Pure OC (600 iter) achieves RMSE~$=0.0053$, comparable
to the hybrid schedule (RMSE~$=\rmseShortRun$),
confirming that OC alone is sufficient for this problem;
the hybrid schedule is retained as the default
configuration for generalisation to other problems
...
OC optimiser & Pure MMA stagnates at $J\approx J_0$ for 600 iter (RMSE~$=\rmsePureMMA$, Table~\ref{tab:mma_comparison}); pure OC and hybrid OC/MMA both converge & Validated by three-way comparison \\ 
...
\caption{Hybrid OC/MMA vs.\ pure MMA: 600-iteration comparison
         on the $20\times5$ mesh, linear gradient target, 600 iterations.
         Pure OC performs comparably to the hybrid schedule;
...
Pure OC (600 iter)       & $0.0053$        & OC bisect & Yes & comparable to hybrid \\
