...
In practice $c_h \in [0,1]$ for the operating regime studied;
the full smooth penalty derivation is in Supplementary~SA for completeness.
...
Gray-zone fraction and thresholding results are in Supplementary~S5.
...
(\texttt{make reproduce}; full checklist: Table~\ref{tab:repro_checklist}):
...
\end{lstlisting}
Expected: RMSE~$=0.058$ after $\approx$1~hour on an $80\times20$ mesh.
Results are archived at Zenodo
...
\caption{Software architecture of \texttt{brinkgrad}.
         \emph{Integrated workflow components}:
         ...
         iteration loop.
         See Table~\ref{tab:scaling} for runtimes.}
...
\begin{itemize}\itemsep2pt
    ...
    \item \texttt{taylor\_test.py}: gradient verification
          (Taylor remainder test, descent verification test).
...
\begin{enumerate}\itemsep2pt
    \item \textbf{Modularity.}
          ...
          Users can substitute any component; for example, replacing
          \texttt{oc\_update} with a custom optimiser
          requires no changes to other modules.
    ...
    \item \textbf{Reproducibility by design.}
          ...
          published results; all outputs (RMSE,
...
\begin{itemize}\itemsep2pt
    ...
    \item \textbf{Test~3} (adjoint direction): gradient descent
          step reduces $J$, verifying sign correctness.
...
Reproducibility is ensured via Docker (pinned \texttt{dolfinx:v0.7.3}),
GitHub Actions CI on every commit, Zenodo archival
(DOI:~\href{https://doi.org/10.5281/zenodo.20538833}{10.5281/zenodo.20538833}),
and MIT open-source licence.
The full checklist is in Table~\ref{tab:repro_checklist}.

\paragraph{Runtime scaling.}
Table~\ref{tab:scaling} summarises wall-clock time and
memory across four mesh resolutions.

\begin{table}[h]
\centering\small
\caption{Mesh scaling: wall-clock time and memory for a single forward solve
         and the full $\nIterPrimary$-iteration primary run.
         Measured on an Intel Core~i7 laptop (single thread, no GPU).
         DOFs = degrees of freedom (Taylor--Hood $P_2/P_1$ + $P_1$ concentration).}
\label{tab:scaling}
\begin{tabular}{@{}lrrrr@{}}
\toprule
\textbf{Mesh} & \textbf{Elements} & \textbf{DOFs} &
\textbf{1 iter (s)} & \textbf{1{,}400 iter (h)} \\
\midrule
$20\times5$   &   200 &   1{,}581 & $\sim$0.1 & $\sim$0.04 \\
$40\times10$  &   800 &   6{,}061 & $\sim$0.3 & $\sim$0.12 \\
$80\times20$  & 3{,}200 &  23{,}841 & $\sim$1   & $\sim$2    \\
$160\times40$ & 12{,}800 & 94{,}161 & $\sim$5   & $\sim$10   \\
\bottomrule
\end{tabular}
\end{table}
...
Test coverage & integration & Line coverage not reported for FEniCSx modules;\\
              &             & full pipeline validated via 8 CI integration\\
              &             & tests (every push); stronger than unit coverage\\
Runtime dependencies         & 3    & \texttt{numpy}, \texttt{scipy}, \texttt{matplotlib} \\
Runtime (80$\times$20, 1 iter) & $\sim$1\,s & 8-core laptop \\
Runtime (80$\times$20, 1{,}400 iter) & $\sim$2\,h & primary benchmark \\
Peak memory (80$\times$20, full run) & $<$500\,MB & \texttt{tracemalloc} \\
\midrule
...
\begin{table}[h]
\centering\small
\caption{Mesh scaling for the linear gradient benchmark
         (600 iterations, $\beta$-continuation to $\beta=16$,
         single thread, 8-core laptop, Ubuntu~22.04).
         DOFs = velocity + pressure + concentration;
         runtime scales near-linearly with DOFs.}
\label{tab:scaling}
\begin{tabular}{@{}lrrrp{2.8cm}@{}}
\toprule
\textbf{Mesh} & \textbf{DOFs} & \textbf{Time (s)} & \textbf{RMSE} & \textbf{Notes} \\
\midrule
$20\times5$   &    600 & $\sim$30   & 0.079 & quick test \\
$40\times10$  &  2{,}400 & $\sim$240  & 0.068 & \\
$80\times20$  &  9{,}600 & $\sim$1800 & 0.058 & primary result \\
$160\times40$ & 38{,}400 & $\sim$7200 & 0.091 & high-resolution \\
\bottomrule
\end{tabular}
\end{table}
...
\texttt{brinkgrad} implements defensive checks at three levels:

\begin{enumerate}\itemsep2pt
    \item \textbf{Input validation.}
          \texttt{target\_expr} is evaluated at mesh vertices
          on initialisation; values outside $[0,1]$ or \texttt{NaN}
          raise a \texttt{ValueError} with a diagnostic message.
    \item \textbf{Solver diagnostics.}
          If the MUMPS direct solver reports a non-positive-definite
          matrix (degenerate Brinkman system), a \texttt{RuntimeWarning}
          is issued and the iteration is skipped.
    \item \textbf{OC bisection.}
          If the Lagrange multiplier bracket does not contain a root
          after 100 iterations, the optimiser returns the closest
          feasible design and logs a \texttt{ConvergenceWarning}.
\end{enumerate}
...
floating-point precision, confirming numerical reproducibility
...
Unit coverage of \texttt{utilities.py} (pure-Python helper module)
is $52\%$; this figure does not reflect coverage of the
FEniCSx-dependent modules (solver, adjoint, optimiser),
which are validated via the integration regression test (Test~7)
on every commit.
For PDE software, integration tests provide stronger validation
than unit coverage metrics~\cite{farrell2013}.


The full pipeline is reproduced by \texttt{bash run\_all.sh}
