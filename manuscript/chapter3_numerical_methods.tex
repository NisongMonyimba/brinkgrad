...
\label{sec:algorithm}
Table~\ref{alg:main} summarises the complete optimisation procedure.

\begin{table}[h]
\centering\small
\caption{Optimisation loop (presented as a table for compatibility):
         hybrid OC/MMA with $\beta$-continuation.
...
Profile gallery (the target profile results (Gaussian peak RMSE~$=\rmseGaussian$)) &
