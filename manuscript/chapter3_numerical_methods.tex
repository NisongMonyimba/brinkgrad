...
\begin{algorithm}[h]
\caption{Hybrid OC/MMA topology optimisation with $\beta$-continuation}
\label{alg:main}
\begin{algorithmic}[1]
\Require{target $c^*(y)$, mesh, $V^*$, $w_f$, $w_c$, $\alpha_{\max,0}=10^3$}
\State Initialise $\rho\leftarrow V^*$ (uniform), reset MMA state
\For{step $k = 0,1,\ldots,\nIterPrimary-1$}
    \State $\beta_k,\,\text{method}_k,\,\alpha_{\max,k}\leftarrow$ schedule($k$)
    \State $\rho_f \leftarrow $ HelmholtzFilter($\rho$)
    \Comment{Eq.~\eqref{eq:helmholtz}}
    \State $\bar\rho \leftarrow $ Heaviside($\rho_f$, $\beta_k$)
    \Comment{Eq.~\eqref{eq:heaviside}}
    \State $(\mathbf{u},p,c) \leftarrow $ ForwardSolve($\bar\rho$, $\alpha_{\max,k}$)
    \Comment{Eqs.~\eqref{eq:brinkman}, \eqref{eq:convdiff}}
    \State $(\mathbf{v},q,\lambda) \leftarrow $ AdjointSolve($\bar\rho$, $\mathbf{u}$, $c$)
    \Comment{Section~\ref{sec:adjoint}}
    \State $s \leftarrow $ Sensitivity($\bar\rho$, $\mathbf{v}$, $\lambda$)
    \If{method$_k$ = OC}
        \State $\rho \leftarrow $ OCupdate($\rho$, $s$, $V^*$)
    \Else
        \State $\rho \leftarrow $ MMAupdate($\rho$, $s$, $V^*$)
    \EndIf
    \State Track best $J$; store $\rho_{\rm best}$
\EndFor
\State \Return $\rho_{\rm best}$
\end{algorithmic}
\end{algorithm}
